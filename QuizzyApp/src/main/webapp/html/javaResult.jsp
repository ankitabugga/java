...
<head>
    ...
    <script>
        function handleMediumQuiz(score) {
            if (score > 5) {
                window.location.href = "mediumQuiz.jsp";
            } else {
                alert("Your score is too low (" + score + "/10). Please repeat the Basic JavaScript Quiz before proceeding.");
            }
        }
    </script>
</head>
...
<div class="container text-center">
    <div class="result-box">
        <h2 class="mb-4">Your Basic JavaScript Quiz Score</h2> 
        <p class="fs-4">You scored: <strong><%= score %>/10</strong></p>

        <!-- Medium Quiz Button -->
        <button class="btn btn-success mt-2" onclick="handleMediumQuiz(<%= score %>)">
            Take Medium JavaScript Quiz
        </button>

        <!-- Repeat Basic Quiz Button (only for low scores) -->
        <% if(score <= 5) { %>
            <a href="javascriptBasic.jsp" class="btn btn-warning mt-2 ms-2">
                Repeat Basic JavaScript Quiz
            </a>
        <% } %>
